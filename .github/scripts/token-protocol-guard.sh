...
SCAN_PATHS=(src docs docs/AST-CORE-CANON.md)
...
for pattern in "${FORBIDDEN_NATIVE[@]}"; do
  ...
  if [ -n "$hits" ]; then
    echo "::error::token-protocol-guard: ERC must not be the protocol source of truth (docs/AST-CORE-CANON.md §VI)."
    echo "$hits"
    fail=1
  fi
done
...
if [ -d src ]; then
  ...
  if [ -n "$code_hits" ]; then
    ...
    if [ -n "$filtered" ]; then
      echo "::error::token-protocol-guard: core src binds to ERC without adapter path (docs/AST-CORE-CANON.md §6.2)."
      echo "$filtered"
      fail=1
    fi
  fi
...
if ! grep -q 'AST Token Protocol' docs/AST-CORE-CANON.md; then
  echo "::error::docs/AST-CORE-CANON.md must define AST Token Protocol."
  fail=1
fi
